            ...
            <form method="POST" action="{{ route('vendeurs.update', $vendeur) }}">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="nom_vend" class="form-control @error('nom_vend') is-invalid @enderror" value="{{ old('nom_vend', $vendeur->nom_vend) }}" required>
                        @error('nom_vend')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label">Prénom <span class="text-danger">*</span></label>
                        <input type="text" name="prenom_vend" class="form-control @error('prenom_vend') is-invalid @enderror" value="{{ old('prenom_vend', $vendeur->prenom_vend) }}" required>
                        @error('prenom_vend')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label">Téléphone <span class="text-danger">*</span></label>
                        <input type="text" name="tel_vend" class="form-control @error('tel_vend') is-invalid @enderror" value="{{ old('tel_vend', $vendeur->tel_vend) }}" required>
                        @error('tel_vend')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-12 mb-3">
                        <label class="form-label">Ferme / Exploitation</label>
                        <input type="text" name="farm_vend" class="form-control @error('farm_vend') is-invalid @enderror" value="{{ old('farm_vend', $vendeur->farm_vend) }}" maxlength="25">
